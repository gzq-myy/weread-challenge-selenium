...
| 容器版 CLI 可运行 | 在仓库根目录执行 `node scripts/runtime-smoke.js docker`，且本机 Docker 可用 | 基于当前 `Dockerfile` 成功构建本地镜像，并通过 `docker run --rm <image> node src/weread-challenge.js -h` 输出 CLI 帮助；输出包含 `run`、`schedule`、`help` |
